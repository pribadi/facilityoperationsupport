...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function db_update_validation()
	{
		...
		// Disabled by Adit due to required fields in edit breaking with readonly implementation
/*		if(!empty($required_fields))
		{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert === null)
			{
				if($this->callback_before_insert !== null)
				{
					...
					elseif(is_object($callback_return)) {
						// Modified for Custom error message
						return $callback_return;
						// Custom error message until here..
					}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function insert_layout($insert_result = false, $error_message = false)
	{
		@ob_end_clean();
		if($insert_result === false)
		{
	...
	protected function load_js_chosen()
	{
		...
		//$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/jquery.chosen.js');
	}
	...
	protected function update_layout($update_result = false, $state_info = null, $error_message = false)
	{
		@ob_end_clean();
	...
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			case 5://insert
				...
				// Modified for Custom error message
				if (is_object($insert_result)) {
					$this->insert_layout($insert_result->status, $insert_result->message);
				}
				else {
			...
			case 6://update
				...
				if (is_object($update_result)) {
					$this->update_layout($update_result->status, $state_info, $update_result->message);
				}
				else {
